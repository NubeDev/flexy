...
func UpdateCasbin(id int, u AddCasbinStruct) bool {
	...
	updates["v1"] = u.V1
	updates["v2"] = u.V2
	err, rowsAffected := model.Update(&model.CasbinRuleM{}, wheres, updates)
	if rowsAffected == 0 {
		log.Println("Failed to modify Casbin!")
		log.Println(err)
		return false
	}
	return true
...
func (c *CasbinStruct) GetAll() ([]*model.CasbinRuleM, error) {
	cas, err := model.GetCasbinRuleList(c.PageNum, c.PageSize, c.getConditionMaps())
	if err != nil {
		return nil, err
	}
