...
func SetLoggedUserInfo(userId uint, refreshToken string) error {
	...
	if rowsAffected == 0 {
		log.Println("Failed to set login information!")
	}
	return nil
}
...
func RefreshAccessToken(RefreshToken string) (map[string]interface{}, error) {
	...
	if user.ID == 0 {
		return data, fmt.Errorf("the user information corresponding to this access_token does not exist")
	}
...
func ChangeUserPassword(userId uint, newPassword string) bool {
	...
	if rowsAffected == 0 {
		log.Println("Failed to update user password!")
		return false
	}
	return true
}
